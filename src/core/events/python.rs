...
impl PyEventList {
    pub(crate) fn new(py: Python<'_>, elements: Vec<PyEvent>) -> Self {
        Self(PyList::new(py, elements).into())
    }
}
...
impl PyEventList {
    ...
    /// `print(events)`.
    fn __repr__<'a>(&'a self, py: Python<'a>) -> PyResult<&'a PyString> {
        let events: &PyAny = self.0.downcast::<PyList>(py).unwrap();
        events.repr()
    }

    /// Allows to use the object as a dictionary, eg. `events[0]`.
    fn __getitem__<'a>(&'a self, py: Python<'a>, attr: usize) -> PyResult<&'a PyAny> {
        let events: &PyAny = self.0.downcast::<PyList>(py).unwrap();
    ...
    fn __len__(&self, py: Python<'_>) -> usize {
        let events = self.0.downcast::<PyList>(py).unwrap();
        events.len()
    }
